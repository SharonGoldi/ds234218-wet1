...
ServersFarm::ServersFarm(int id,int m):
        farm_id(id),num_of_servers(m),linux_in_use(0),win_in_use(0){
    this->linux_node = new ServerOSKey(id, m);
    this->windows_node = new ServerOSKey(id, 0);
...
ServersFarm::~ServersFarm() {
    delete this->windows_node;
    delete this->linux_node;
